...
const taskSchema = z
  .object({
    ...
    estimatedMaterialsCostLumpSum: z
      .number()
      .min(0, 'Lump sum cost cannot be negative')
      .nullable()
      .optional(),
...
export const QuoteDetailModal: React.FC<QuoteDetailModalProps> = ({
  ...
  function mapQuoteDataToFormValues(data: QuoteData): QuoteDetailFormValues {
    return {
      ...
      notes: data.notes,
      tasks:
        data.tasks?.map((task: QuoteData['tasks'][number]) => ({
          id: task.id,
          ...
          estimatedMaterialsCostLumpSum: task.estimatedMaterialsCost
            ? Number(task.estimatedMaterialsCost)
            : null,
          materials:
            task.materials?.map((mat: QuoteData['tasks'][number]['materials'][number]) => ({
              id: mat.id,
  ...
  function formatFormValuesToApiInput(values: QuoteDetailFormValues, id: string): QuoteUpdateInput {
    ...
    return {
      ...
      tasks:
        dataToUpdate.tasks?.map((task: QuoteDetailFormValues['tasks'][number]) => ({
          ...
          estimatedMaterialsCostLumpSum:
            task.materialType === 'lumpsum'
              ? task.estimatedMaterialsCostLumpSum || null
              : undefined,
          materials:
            task.materialType === 'itemized'
              ? task.materials?.map(
                  (mat: QuoteDetailFormValues['tasks'][number]['materials'][number]) => ({
                    id: mat.id || undefined,
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }: { value: QuoteDetailFormValues }) => {
      ...
      if (!result.success) {
        ...
        toast.error(
          t('common.validationErrorToast', { defaultValue: 'Please fix validation errors.' })
        );
        return;
      }
  ...
  return (
    <Modal size="5xl" isOpen={isOpen} onClose={onClose} scrollBehavior="inside">
      <ModalContent>
        ...
        {/* <form.Provider> */}
        <ModalHeader>
          {t('quotes.editModalTitle')} {`(#${quoteData.sequentialId})`}
        </ModalHeader>
        <ModalBody>
          <form
          ...
          >
            ...
            <form.Field
              ...
              children={(field) => (
                <>
                  <TaskList form={form} field={field} readOnly={updateQuoteMutation.isPending} />
                  <Button
                    type="button"
                    onClick={() =>
                      field.pushValue({
            ...
            {/* --- Charges & Summary --- */}
            <div className="mb-4 grid grid-cols-1 items-end gap-4 md:grid-cols-3">
