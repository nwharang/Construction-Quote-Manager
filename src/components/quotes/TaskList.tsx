    ...
    <Card className="mb-6">
      ...
      <CardBody>
        {field.state.value.length === 0 ? (
          <div className="py-4 text-center text-gray-500">{t('quotes.noTasksAddedEditable')}</div>
        ) : (
          <div className="space-y-6">
            {field.state.value.map(
              (task: QuoteDetailFormValues['tasks'][number], index: number) => (
                <div
                  key={task.id || index} // Use task.id directly or index as fallback
                ...
                >
                  ...
                  <form.Field
                    ...
                    form={form}
                    children={(
                      taskField: any // Explicitly type as any
                    ) => (
                  ...
                  {field.state.value[index]?.materialType === 'itemized' && (
                    <div className="mt-4 mb-4 border-l-2 border-gray-200 pl-4">
                      ...
                      <form.Field
                        ...
                        form={form}
                        children={(
                          materialsField: any // Explicitly type as any
                        ) => (
                          <div className="space-y-3">
                            {materialsField.state.value?.map(
                              (
                                material: any,
                                materialIndex: number // Type material as any
                              ) => (
                                <div
                                  key={`${index}-${materialIndex}`}
                                  className="relative flex flex-col items-start space-y-2 rounded-md border p-3 md:flex-row md:items-center md:space-y-0 md:space-x-2"
                                >
                                  ...
                                  <div className="grid w-full grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-5">
                                    <form.Field
                                      name={`tasks[${index}].materials[${materialIndex}].productId`}
                                      form={form}
                                      children={(
                                        matField: any // Explicitly type as any
                                      ) => (
                                        <div className="sm:col-span-2">
                                          <ProductSelector
                                            ...
                                            onChange={(productId) =>
                                              matField.handleChange(productId)
                                            }
                                            disabled={readOnly}
                                          />
                                        </div>
                                    ...
                                    />
                                    <form.Field
                                      name={`tasks[${index}].materials[${materialIndex}].quantity`}
                                      form={form}
                                      children={(
                                        matField: any // Explicitly type as any
                                      ) => (
                                        <NumberInput
                                          ...
                                          value={matField.state.value}
                                          onChange={(v) => matField.handleChange(v ?? 1)}
                                          onBlur={matField.handleBlur}
                                          min={1}
                                          step={1}
                                    ...
                                    />
                                    <form.Field
                                      name={`tasks[${index}].materials[${materialIndex}].unitPrice`}
                                      form={form}
                                      children={(
                                        matField: any // Explicitly type as any
                                      ) => (
                                        <NumberInput
                                          ...
                                          value={matField.state.value}
                                          onChange={(v) => matField.handleChange(v ?? 0)}
                                          onBlur={matField.handleBlur}
                                          min={0}
                                          step={0.01}
                                          formatOptions={{ style: 'currency', currency: 'USD' }}
                                    ...
                                    />
                                    <form.Field
                                      name={`tasks[${index}].materials[${materialIndex}].notes`}
                                      form={form}
                                      children={(
                                        matField: any // Explicitly type as any
                                      ) => (
                                        <Textarea
                                          label={t('quotes.materialNotesLabel')}
                                          placeholder={t('quotes.materialNotesPlaceholder')}
                                          value={matField.state.value ?? ''}
                                          onChange={(e) =>
                                            matField.handleChange(e.target.value || null)
                                          }
                                          onBlur={matField.handleBlur}
                                          rows={1}
                                          isDisabled={readOnly}
      ...
      {!readOnly && (
        <CardFooter>
          <Button
            ...
            variant="ghost"
            onClick={() =>
              field.pushValue({ description: '', price: 0, materialType: 'lumpsum', materials: [] })
            }
          >
