...
import { getUserFromDb } from '~/server/utils/get-user-from-db';
...
export const authRouter = createTRPCRouter({
  ...
  login: publicProcedure.input(loginUserSchema).mutation(async ({ input }) => {
    try {
      ...
      // 2. Use the utility function to find and validate user
      const user = await getUserFromDb(email, password);

      // 3. Handle authentication failure (user not found or password mismatch)
      if (!user) {
        throw new TRPCError({
          code: 'UNAUTHORIZED',
      ...
      // 4. Return successful login response (user data needed for session)
      return {
        status: 'success',
        user: {
          id: user.id,
      ...
      // 5. Handle errors
      console.error('Error logging in (tRPC):', error);
      if (error instanceof TRPCError) throw error;
      throw new TRPCError({
        code: 'INTERNAL_SERVER_ERROR',
