...
type SettingsResponse = RouterOutput['settings']['get'];
type UpdateSettingsInput = RouterInput['settings']['update'];
...
export default function SettingsPage() {
  ...
  const { settings, setSettings, isLoading, setLoading, isUpdating, setUpdating } =
    useConfigStore();

  // Local state for settings that should only be applied when saved
  const [localSettings, setLocalSettings] = React.useState({
    locale: settings.locale || 'en',
    isDarkMode: settings.theme === 'dark',
  });

  // Update local settings when settings are loaded from the server
  useEffect(() => {
    setLocalSettings({
      locale: settings.locale || 'en',
      isDarkMode: settings.theme === 'dark',
  ...
  useEffect(() => {
    if (settingsQuery.data) {
      ...
      setLocalSettings({
        locale: formattedSettings.locale || 'en',
        isDarkMode: formattedSettings.theme === 'dark',
      });
    }
  ...
  const handleSaveSettings = () => {
    ...
    const updatePayload: UpdateSettingsInput = {
      ...
      theme: (localSettings.isDarkMode ? 'dark' : 'light') as 'system' | 'light' | 'dark',
      locale: localSettings.locale as 'en' | 'vi' | 'es',
  ...
  // Handle input changes for immediate settings
  const handleInputChange = (field: string, value: string | number | boolean) => {
    setSettings({ [field]: value });
  };

  // Handle input changes for deferred settings (applied on save)
  const handleLocalSettingChange = (field: string, value: string | number | boolean) => {
    ...
    <>
      <Card>
        <CardHeader className="flex flex-col gap-2">
          ...
          <p className="text-default-500 text-sm sm:text-base">{t('app.tagline')}</p>
        </CardHeader>
        <Divider />
        <CardBody>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                  ...
                  <div className="flex items-center gap-2">
                    <LocaleSwitch
                      ...
                      onLocaleChange={(locale) => handleLocalSettingChange('locale', locale)}
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
                  ...
                  <div className="flex items-center gap-2">
                    <ThemeToggle
                      ...
                      onThemeChange={(isDark) => handleLocalSettingChange('isDarkMode', isDark)}
                      size="md"
                    />
                    <span className="inline-block text-sm">
                      {localSettings.isDarkMode ? t('theme.dark') : t('theme.light')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange('defaultMarkupCharge', parseFloat(e.target.value))
                    }
                    endContent="%"
                    min={0}
                    max={100}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange('defaultComplexityCharge', parseFloat(e.target.value))
                    }
                    endContent="%"
                    min={0}
                    max={100}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange('defaultTaskPrice', parseFloat(e.target.value))
                    }
                    startContent="$"
                    min={0}
                    step={0.01}
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange('defaultMaterialPrice', parseFloat(e.target.value))
                    }
                    startContent="$"
                    min={0}
                    step={0.01}
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <Button
              variant="light"
              onPress={() => {
                // Reset local settings to current settings
                setLocalSettings({
                  locale: settings.locale || 'en',
                  isDarkMode: settings.theme === 'dark',
                });
